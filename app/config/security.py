...
class RateLimitConfig(BaseSettings):
    ...
    block_duration: int = 10  # Duración de bloqueo en segundos (reducido para no afectar servicio)
    alert_threshold: float = 0.85  # Umbral para alertas (más permisivo)
    
    # Configuración personalizada de endpoints
    endpoint_config_json: Optional[str] = Field(default=None, alias="RATE_LIMIT_ENDPOINT_CONFIG")
    ...
    def default_endpoint_limits(self) -> Dict[str, Dict[str, int]]:
        ...
        return {
            ...
            "POST /api/v1/chats/*/messages": {"rpm": 200, "rph": 3000},  # Enviar mensajes (muy frecuente)
            "GET /api/v1/chats/*/messages": {"rpm": 300, "rph": 4000},  # Leer mensajes (muy frecuente)
    ...
    def get_endpoint_limits(self, method: str, path: str) -> Dict[str, int]:
        ...
        for pattern, limits in default_config.items():
            if "*" in pattern:
                ...
                if method == pattern_method and self._match_wildcard_path(path, pattern_path):
                    return limits
