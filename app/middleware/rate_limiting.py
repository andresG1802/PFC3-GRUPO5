...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app):
        ...
        logger.info(f"RateLimitingMiddleware inicializado - Enabled: {self.config.enabled}")
    ...
    async def _init_redis(self):
        ...
        try:
            ...
            logger.warning("Rate limiting funcionará en modo degradado (sin persistencia)")
            ...
            try:
                ...
                if scheme.lower() == "bearer" and token:
                    ...
                    if payload and "sub" in payload:
                        ...
                        user_hash = hashlib.sha256(payload["sub"].encode()).hexdigest()[:16]
    ...
    async def _check_rate_limit(self, client_id: str, endpoint: str, rpm_limit: int, rph_limit: int) -> Tuple[bool, Dict]:
        """
        ...
        try:
            ...
            limit_info = {
                ...
                "severe_violation": minute_count > rpm_limit * 2 or hour_count > rph_limit * 1.5
            }
    ...
    async def _apply_temporary_block(self, client_id: str, endpoint: str):
        ...
        try:
            ...
            logger.warning(f"Cliente {client_id} bloqueado temporalmente por {self.config.block_duration} minutos")
    ...
    def _add_rate_limit_headers(self, response: Response, limit_info: Dict):
        ...
        response.headers["X-RateLimit-Requests-Per-Minute-Limit"] = str(limit_info.get("remaining_rpm", 0) + 1)
        response.headers["X-RateLimit-Requests-Per-Minute-Remaining"] = str(limit_info.get("remaining_rpm", 0))
        response.headers["X-RateLimit-Requests-Per-Minute-Reset"] = str(limit_info.get("reset_time_rpm", 0))
        
        # Headers para límites por hora (más descriptivos)
        response.headers["X-RateLimit-Requests-Per-Hour-Limit"] = str(limit_info.get("remaining_rph", 0) + 1)
        response.headers["X-RateLimit-Requests-Per-Hour-Remaining"] = str(limit_info.get("remaining_rph", 0))
        response.headers["X-RateLimit-Requests-Per-Hour-Reset"] = str(limit_info.get("reset_time_rph", 0))
