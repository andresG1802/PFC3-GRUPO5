...
with patch('redis.Redis') as mock_redis, \
    ...
    mock_waha_instance.get_messages = AsyncMock(return_value={"messages": [], "total": 0})
    mock_waha_instance.send_message = AsyncMock(return_value={"id": "test_msg", "status": "sent"})
    mock_waha_instance.get_session_status = AsyncMock(return_value={
        "name": "default",
        "status": "WORKING"
    })
    mock_waha_instance.close = AsyncMock()
    mock_waha_instance._normalize_chat_data = MagicMock(return_value={
        "id": "test@c.us",
        "name": "Test Chat",
        "type": "individual",
        "timestamp": None,
...
def mock_waha_client():
    ...
    mock._normalize_chat_data = MagicMock(return_value={
        "id": "test@c.us",
        "name": "Test Chat",
        "type": "individual",
        "timestamp": None,
